...
| `apps/scoreboard` | Lemon Hunt's shared leaderboard: Vercel functions over a free Neon Postgres, or the same handler as a long-running server (Render, Railway) — its own module, so pgx reaches no importer |
