...
| **[`examples/showcase`](../examples/showcase)** | **Flagship Multi-Tab Showcase** | Flagship application featuring tabs (Home, Settings, Graphics, Playground, Reference), 3D shaders, glassmorphism modals, and theme switching. | `go run ./examples/showcase` |
| **[`examples/simple`](../examples/simple)** | **Minimal Single-Import App** | Minimal starter example using the unified `github.com/thebanri/limoni` API with zero boilerplate. | `go run ./examples/simple` |
| **[`examples/agent_checklist`](../examples/agent_checklist)** | **Agent-Driven Checklist** | A release checklist meant to be driven by an AI agent through `cmd/limoni-mcp`; its deploy token field is secret, so the agent can type into it but never read it back. | `go run -tags limoni_debug ./examples/agent_checklist` |
...
| **[`examples/charts`](../examples/charts)** | **Data Visualization Studio** | Standalone showcase of `LineChart` (subpixel curves), `BarChart` (vertical spectrum), and `PieChart` (donut distribution). | `go run ./examples/charts` |
| **[`examples/treeview`](../examples/treeview)** | **File Tree Explorer** | Hierarchical collapsible tree (`widgets.TreeView`), folder icons, guide lines, and file metadata preview. | `go run ./examples/treeview` |
| **[`examples/toast`](../examples/toast)** | **Toast Notification Studio** | Stackable notifications (`widgets.ToastManager`), Info/Success/Warning/Error alerts, corner positioning, and auto-dismiss. | `go run ./examples/toast` |
...
| **[`examples/demo`](../examples/demo)** | **Complete Showcase** | Multi-tab flagship demo featuring 3D models, system gauges, DevTools HUD (`F12`), forms, and command palettes. | `go run ./examples/demo` |
| **[`examples/xray`](../examples/xray)** | **Diff X-Ray** | A night city — stars, a train, lit windows, water — that an X-ray sweeps across to show which cells the diff actually sends, with bytes measured by the real encoder against a full repaint. Space freezes the city: nothing changes, nothing is sent. `-film` plays a 20-second loop for recording. | `go run ./examples/xray` |
