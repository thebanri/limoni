...
| **[`examples/agent_checklist`](../../examples/agent_checklist)** | **Ajanla Sürülen Kontrol Listesi** | `cmd/limoni-mcp` üzerinden bir yapay zekâ ajanı tarafından sürülmek için yazılmış sürüm kontrol listesi; deploy token alanı gizli olduğundan ajan içine yazabilir ama geri okuyamaz. | `go run -tags limoni_debug ./examples/agent_checklist` |
...
| **[`examples/xray`](../../examples/xray)** | **Diff Röntgeni** | Yıldızlar, bir tren, yanan pencereler ve su yansımasıyla bir gece şehri; üzerinden geçen röntgen, diff'in gerçekte hangi hücreleri gönderdiğini gösterir ve gerçek kodlayıcıyla ölçülen baytları tam yeniden çizimle karşılaştırır. Boşluk şehri dondurur: hiçbir şey değişmez, hiçbir şey gönderilmez. `-film` kayıt için 20 saniyelik bir döngü oynatır. | `go run ./examples/xray` |
