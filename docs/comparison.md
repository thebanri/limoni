...
| **Double Buffering & Diff** | **Dirty-cell diff + adaptive flush, `ECH`/`EL`/`REP`, scroll regions (`DECSTBM` + `SU`/`SD`) and `ICH`/`DCH`**¹ | None (entire strings dumped to stdout) | Cell diff + `ECH`/`REP`/`ICH`/`DCH` + scroll optimization | Double-buffered diff |
...
| **3D & Vector Graphics**| **Built-in 3D (OBJ/STL/PLY/GLB), depth-buffered flat/Lambert/Gouraud/texture shading, drawn as Braille/sextant dots or sent as a picture over kitty/iTerm2/Sixel** | Third-party / custom | Third-party / custom | Addons required |
