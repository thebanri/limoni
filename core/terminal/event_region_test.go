...
func TestEventRegionMetadataAndDisabledFiltering(t *testing.T) {
	...
	if !frame.DispatchEventRegions(driver.MouseEvent{X: 2, Y: 1, Button: driver.MouseLeft}) {
...
func TestEventRegionEnterLeave(t *testing.T) {
	frame := NewFrame(buffer.NewBuffer(cell.NewRect(0, 0, 20, 4)), NewFocusManager())
	transitions := make([]driver.PointerEventKind, 0, 2)
	frame.RegisterEventRegion(EventRegion{
	...
	frame.DispatchPointerMove(driver.MouseEvent{X: 2, Y: 2, Button: driver.MouseNone})
	frame.DispatchPointerMove(driver.MouseEvent{X: 10, Y: 2, Button: driver.MouseNone})
	if len(transitions) != 2 || transitions[0] != driver.PointerEnter || transitions[1] != driver.PointerLeave {
...
func TestEventRegionDoubleClick(t *testing.T) {
	...
	frame.DispatchClick(driver.MouseEvent{X: 2, Y: 2, Button: driver.MouseLeft}, start)
	frame.DispatchClick(driver.MouseEvent{X: 2, Y: 2, Button: driver.MouseLeft}, start.Add(200*time.Millisecond))
