...
func (p *Program) RunTerminal(ctx context.Context, term *terminal.Terminal, b *driver.Backend) error {
	...
	for {
		...
		case event, ok := <-b.Events():
			...
			if !p.catchCtrlC && event.Type == driver.EventKey && event.Key.Ctrl && (event.Key.Ch == 'c' || event.Key.Ch == 'C') {
				p.Stop()
				return nil
			}
...
// WithCatchCtrlC configures whether Ctrl+C is caught and forwarded to the model
// as a KeyPressMsg instead of causing RunTerminal to terminate.
...
// SendDriver converts a low-level driver event and injects its typed message.
func (p *Program) SendDriver(ctx context.Context, event driver.Event) error {
	message := MessageFromDriver(event)
	if message == nil {
		return nil
	}
...
// SendBackend is a backward-compatible alias for SendDriver.
func (p *Program) SendBackend(ctx context.Context, event driver.Event) error {
	return p.SendDriver(ctx, event)
}
