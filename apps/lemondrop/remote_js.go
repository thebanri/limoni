...
// leaves the function at window.__lemondrop_board_changed; without it, as
// under Node, nothing happens.
func boardChanged() {
	if f := js.Global().Get("__lemondrop_board_changed"); f.Type() == js.TypeFunction {
		f.Invoke()
	}
}

// boardURL is the shared leaderboard's address, which the page leaves at
// window.__lemondrop_scoreboard; the flag has no say in a browser.
func boardURL(string) string {
	v := js.Global().Get("__lemondrop_scoreboard")
