...
func drawPlaygroundGrid(t *terminal.Terminal, f *terminal.Frame, state *AppState, accentColor cell.Color, area cell.Rect) {
	...
	imgBlock := widgets.Block{
		...
		Child:          widgets.Image{Img: profileImg, CircleMask: state.ProfileFrame == "Rounded", ForceHalfBlock: false, OpaqueBackground: true, Background: cell.NewColorRGB(25, 28, 36), Opacity: float64(state.AvatarOpacityState.Value) / 100.0},
	}
